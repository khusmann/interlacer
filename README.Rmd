...
missing. Many textual tabular data sources will encode these missing reasons as
special values *interlaced* with the valid values in a column (e.g. `N/A`,
`REFUSED`, `-99`, etc.). Unfortunately, the missing reason information is 
...
character vectors and doing a bunch of string operations and type conversions
...
As it turns out, this form gives us an extremely powerful and expressive way
to simultaneously work with values and missing reasons, as described in 
`vignette("interlacer")`G. (TLDR: It allows us to interact with the variable
as a type-safe [`Result` type](https://en.wikipedia.org/wiki/Result_type),
...
in a tidy and type-safe manner. For example, if I wanted to get a 
