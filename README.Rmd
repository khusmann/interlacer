...
interlacer provides a convenient and typesafe API for loading and manipulating
textual tabular data sources interlaced values and missing reasons. This library
is currently in its experimental stages, so be aware that its interface is 
likely to change in the future. In the meantime, please try it out and
...
* `read_interlaced_csv2()`: semicolon-separated values with `,` as the decimal
   mark
* `read_interlaced_delim()`: delimited files (CSV and TSV are important special
   cases)
