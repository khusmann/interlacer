...
When you need more fine-grained control over value and na channel types, you
can use an `x_cols()` specification, an extension of readr's `readr::cols()`
system. With `x_cols()`, you can control both the value channel and na channel
types in the columns of the resulting data frame. 
...
(df_stress <- read_interlaced_csv(
  ...
  col_types = x_cols(
    person_id = x_col(
      v_col_integer(),
      na_col_factor("REFUSED", "OMITTED", "N/A")
    ),
    current_stress = x_col(
      v_col_factor(levels = c("LOW", "MODERATE", "HIGH")),
      na_col_factor("REFUSED", "OMITTED", "N/A", "DONT_KNOW", "DONT_UNDERSTAND")
    ),
    time_management = x_col(
      v_col_factor(levels = c("POORLY", "FAIRLY_WELL", "WELL", "VERY_WELL")),
      na_col_factor("REFUSED", "OMITTED", "N/A", "NA_VACATION", "NA_OTHER")
    )
  )
))
```
...
equivalent to `readr::col_character()`, etc.
...
5. `na_col_cfactor()`: Use a `cfactor()` na channel. (see `vignette(coded-data)`
for )

The following example shows some of these collectors in action:
...
similarly used to specify a default missing reason collector.

When value collectors are specified without missing reason collectors, they use
`na_col_default()` by default.
