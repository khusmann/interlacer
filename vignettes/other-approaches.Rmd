...
```{r}
sum(declared(c(1, 2, 3, -99, -98), na_values = c(-99, -98)), na.rm=TRUE)
```
...
apply a function to a particular channel, without touching the values of the
other channel, which are especially useful in tidy pipelines.

Finally, because the `interlaced` type is based on a `vctrs::vctrs_vctr`, it
